...
from application.models.Restaurant import CustomerInventory,db
...
class CustomerInventoryController:
    app=None
    def __init__(self):
    ...
    def get_all_free(self, d_json):
        try:
            query = CustomerInventory.query
            if "id" in d_json.keys():
                query = query.filter(CustomerInventory.id == d_json["id"])
            if "id_admin" in d_json.keys():
                query = query.filter(CustomerInventory.id_admin == d_json["id_admin"])            
            if "desc" in d_json.keys():
                query = query.order_by(desc(CustomerInventory.id))
    ...
    def insert(self, data):
        ...
        prepared_data = CustomerInventory(
                id_admin = id_admin,
                name = name,
    ...
    def delete(self, data_filter):
        ...
        if "id" in data_filter.keys():
            query = query.filter(CustomerInventory.id == data_filter["id"])
        if "id_admin" in data_filter.keys():
            query = query.filter(CustomerInventory.id_admin == data_filter["id_admin"])
    ...
    def update(self, id, data):
        response = CustomerInventory.query.filter(CustomerInventory.id == id, CustomerInventory.id_admin == data["id_admin"]).first()
        del(data["id_admin"])
        if response:
