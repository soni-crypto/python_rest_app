const createSupplier = () =>{
    ...
    btn_start.addEventListener("click", ()=>{
        form_customer.reset();
        title_form.textContent = "Crear nuevo proveedor";
    ...
    form_customer.addEventListener("submit", async(e)=>{
        ...
        if (form_customer.dataset.mode === "create"){
            ...
            await fetch("/honey/inventory/supplier/create", {
                method:"POST",
                body:form_data,
...
const updateSupplier = () =>{
    ...
    btn_edit_customer_all.forEach(btn =>{
        btn.addEventListener("click", async()=>{
            i = btn.dataset.i;
            if (i){
                await fetch("/honey/inventory/supplier/read?i="+String(i)).then(res => res.json()).then(data =>{
                    if (data){
                        const resp = JsonToForm("form-customer", data);
                        if (resp === "ok"){
                            btn_update.textContent = "Actualizar";
                            form_customer.dataset.mode = "update";
                            title_form.textContent = "Actualizar datos del proveedor";
    ...
    form_customer.addEventListener("submit", async(e)=>{
        ...
        if (form_customer.dataset.mode === "update"){
            ...
            await fetch("/honey/inventory/supplier/update", {
                method:"POST",
                body:form_data,
            }).then(res => res.json()).then(data=>{
...
const deleteSupplier = () =>{
    ...
    btn_confirm.addEventListener("click", async()=>{
        ...
        await fetch("/honey/inventory/supplier/delete?i="+String(i)).then(res => res.json()).then((data) =>{
            tr_selected.remove();
            closeModal(modal_confirm);
